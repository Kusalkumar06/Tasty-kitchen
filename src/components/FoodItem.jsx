...
function FoodItem(props) {
    ...
    const {food,restaurant_name} = props;
    ...
    <div className="w-[100%] md:w-[45%] lg:w-[30%] my-[20px] flex justify-between p-[10px] md:p-3 shadow-lg rounded-lg md:mx-[15px]">
      ...
      <div className="w-[45%] ml-3 flex flex-col justify-between md:justify-around">
        ...
        <div className="Add to Cart Button Container.">
          {count === 0 ? (
            <button onClick={() => { dispatch(actions.addToCart({ id, name, image_url, cost,restaurant_name })); }} className="border-1 bg-[none] border-[#FFA412] px-[10px] cursor-pointer rounded-lg">Add</button>
          ) : (
            <div className="flex">
              <div className="flex items-center justify-around">
                ...
                <button onClick={() => { dispatch(actions.addToCart({ id, name, image_url, cost,restaurant_name })); }} className="border-1 rounded-sm px-2 cursor-pointer">+</button>
              </div>
            </div>
          )}
        </div>
